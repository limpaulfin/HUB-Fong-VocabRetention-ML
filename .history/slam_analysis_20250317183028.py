...
- extracted_data/data_en_es/en_es.slam.20190204.train
- extracted_data/data_en_es/en_es.slam.20190204.dev
- extracted_data/data_en_es/en_es.slam.20190204.dev.key
...
TRAIN_PATH = "extracted_data/data_en_es/en_es.slam.20190204.train"
DEV_PATH = "extracted_data/data_en_es/en_es.slam.20190204.dev"
DEV_KEY_PATH = "extracted_data/data_en_es/en_es.slam.20190204.dev.key"
...
# Hàm tạo báo cáo kết quả
def generate_report(results, importance, corr_matrix):
    """Tạo báo cáo kết quả phân tích"""
    report_path = os.path.join(RESULTS_DIR, 'analysis_report.md')

    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("# Báo cáo phân tích dữ liệu SLAM\n\n")

        f.write("## 1. Kết quả đánh giá mô hình\n\n")
        f.write("### Logistic Regression\n\n")
        f.write(f"- Accuracy: {results['Logistic Regression']['accuracy']:.4f}\n")
        f.write(f"- Precision: {results['Logistic Regression']['precision']:.4f}\n")
        f.write(f"- Recall: {results['Logistic Regression']['recall']:.4f}\n")
        f.write(f"- F1 Score: {results['Logistic Regression']['f1']:.4f}\n")
        f.write(f"- ROC AUC: {results['Logistic Regression']['roc_auc']:.4f}\n\n")

        f.write("### Random Forest\n\n")
        f.write(f"- Accuracy: {results['Random Forest']['accuracy']:.4f}\n")
        f.write(f"- Precision: {results['Random Forest']['precision']:.4f}\n")
        f.write(f"- Recall: {results['Random Forest']['recall']:.4f}\n")
        f.write(f"- F1 Score: {results['Random Forest']['f1']:.4f}\n")
        f.write(f"- ROC AUC: {results['Random Forest']['roc_auc']:.4f}\n\n")

        f.write("## 2. Tầm quan trọng của đặc trưng\n\n")
        f.write("### Logistic Regression\n\n")
        f.write("| Đặc trưng | Tầm quan trọng |\n")
        f.write("|-----------|---------------|\n")
        for _, row in importance['Logistic Regression'].head(10).iterrows():
        ...
        f.write("| Đặc trưng | Tầm quan trọng |\n")
        f.write("|-----------|---------------|\n")
        for _, row in importance['Random Forest'].head(10).iterrows():
            f.write(f"| {row['Feature']} | {row['Importance']:.4f} |\n")
        f.write("\n")

        f.write("## 3. Tương quan giữa các đặc trưng\n\n")
        f.write("| Đặc trưng | num_attempts | correct_ratio | time_since_last_attempt | days | target |\n")
        f.write("|-----------|--------------|---------------|-------------------------|------|--------|\n")
        for feature in corr_matrix.index:
            f.write(f"| {feature} | {corr_matrix.loc[feature, 'num_attempts']:.2f} | {corr_matrix.loc[feature, 'correct_ratio']:.2f} | {corr_matrix.loc[feature, 'time_since_last_attempt']:.2f} | {corr_matrix.loc[feature, 'days']:.2f} | {corr_matrix.loc[feature, 'target']:.2f} |\n")
        f.write("\n")

        f.write("## 4. Kết luận\n\n")

        # So sánh hiệu suất mô hình
        if results['Random Forest']['accuracy'] > results['Logistic Regression']['accuracy']:
            f.write("- Mô hình Random Forest có hiệu suất tốt hơn Logistic Regression trong việc dự đoán khả năng ghi nhớ từ vựng.\n")
        else:
            f.write("- Mô hình Logistic Regression có hiệu suất tốt hơn Random Forest trong việc dự đoán khả năng ghi nhớ từ vựng.\n")

        # Đặc trưng quan trọng nhất
        lr_top_feature = importance['Logistic Regression'].iloc[0]['Feature']
        rf_top_feature = importance['Random Forest'].iloc[0]['Feature']

        f.write(f"- Đặc trưng quan trọng nhất theo Logistic Regression là '{lr_top_feature}'.\n")
        f.write(f"- Đặc trưng quan trọng nhất theo Random Forest là '{rf_top_feature}'.\n\n")

        # Tương quan
        target_corr = corr_matrix['target'].drop('target').sort_values(ascending=False)
        f.write("- Các đặc trưng có tương quan mạnh nhất với khả năng ghi nhớ từ vựng:\n")
        for feature, corr in target_corr.items():
            f.write(f"  - {feature}: {corr:.2f}\n")

    print(f"Đã tạo báo cáo phân tích tại: {report_path}")
    return report_path
...
def main():
    ...
    # Tạo báo cáo kết quả
    print("Đang tạo báo cáo kết quả...")
    report_path = generate_report(results, importance, corr_matrix)

    # Lưu mô hình
    print("Đang lưu mô hình...")
    joblib.dump(results['Logistic Regression']['model'], os.path.join(RESULTS_DIR, 'logistic_regression_model.pkl'))
    joblib.dump(results['Random Forest']['model'], os.path.join(RESULTS_DIR, 'random_forest_model.pkl'))

    print("\nPhân tích hoàn tất. Các kết quả đã được lưu trong thư mục:", RESULTS_DIR)
    print(f"- {os.path.join(RESULTS_DIR, 'feature_importance.png')}: Biểu đồ tầm quan trọng của đặc trưng")
    print(f"- {os.path.join(RESULTS_DIR, 'confusion_matrix.png')}: Ma trận nhầm lẫn")
    print(f"- {os.path.join(RESULTS_DIR, 'model_comparison.png')}: So sánh hiệu suất mô hình")
    print(f"- {os.path.join(RESULTS_DIR, 'correlation_matrix.png')}: Ma trận tương quan giữa các đặc trưng")
    print(f"- {report_path}: Báo cáo phân tích")
